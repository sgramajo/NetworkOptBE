...
public class Collaborative {
	...
	public GlobalItemItem getRating(String productID, String userID){
		...
		HashMap<String, Double> users2 = ratings2.get(productID); 
		Double firstResult = users.get(userID); 
		Double secondResult = users2.get(userID); 
		GlobalItemItem result = new GlobalItemItem(df2.format(firstResult), df2.format(secondResult)); 
		return result; 
	}
	...
	public static Accuracy testAccuracy(){
		...
		ArrayList<String> moviesList = new ArrayList<>(Arrays.asList("Mulan","Moana","Hercules","Pocahontas","Zootopia","Meet the Robinsons", "The Jungle Book",
		...
		List<List<Double>> accuracyArray = mapToArray(actualRatings, moviesList, userList); 
		List<List<Double>> missingArray = mapToArray(missingRatings, moviesList, userList);
		List<List<Double>> globalArray = mapToArray(estimatedRatingsGB, moviesList, userList); 
		List<List<Double>> itemToItem = mapToArray(estimatedRatingsCF, moviesList, userList); 
		Accuracy results = new Accuracy(accuracyArray, missingArray, globalArray, itemToItem); 
		return results; 
	}

	private static List<List<Double>> mapToArray(HashMap<String, HashMap<String, Double>> results, ArrayList<String> movies, ArrayList<String> users) {
		List<List<Double>> array = new ArrayList<List<Double>>();
		//loop a Map
		for (int j = 0; j < movies.size(); j++) {
			//System.out.println("Key : " + entry.getKey() + " Value : " + entry.getValue());
			List<Double> temp = new ArrayList<Double>(); 
			HashMap<String, Double> mapResults = results.get(movies.get(j)); 
			for (int i = 0; i < users.size(); i++) {
				Double resultNumber = mapResults.get(users.get(i));
				String stringResult = df2.format(resultNumber); 
				temp.add(Double.parseDouble(stringResult)); 
			}
			array.add(temp); 
		}
